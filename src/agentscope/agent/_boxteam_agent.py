...
from ..memory import InMemoryMemory, LongTermMemoryBase, MemoryBase
from ..message import (AudioBlock, ImageBlock, Msg, TextBlock, ToolResultBlock,
...
class CustomReActAgent(ReActAgentBase):
    ...
    def __init__(
        ...
        # -------------- Logging --------------
        self.log_print_file_path = f"data/temp/{self.name}.txt"
        # ensure log directory exists
        try:
            os.makedirs(os.path.dirname(self.log_print_file_path), exist_ok=True)
        except Exception:  # pragma: no cover
            pass
        # reset/clear the log file on every initialization
        try:
            with open(self.log_print_file_path, "w", encoding="utf-8"):
                pass
        except Exception:  # pragma: no cover
            pass
    ...
    def _print_text_block(
        ...
        if len(to_print) > len(text_prefix):
            out_str = to_print[len(text_prefix) :]
            # redirect to log file
            try:
                with open(self.log_print_file_path, "a", encoding="utf-8") as f:
                    f.write(out_str)
            except Exception:  # pragma: no cover
                print(out_str, end="")
